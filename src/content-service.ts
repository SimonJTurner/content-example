...
export function getEntriesByType(entryType: string, locale: LanguagesEnum): Promise<ContentResponseCollection> {
    return fetch(`${process.env.CONTENT_SERVICE}/content/entries?spaceId=${process.env.SPACE_ID}&environmentId=${process.env.ENV_ID}&contentfulApiKey=${process.env.CONTENTFUL_API_KEY}&api-version=1.0&content_type=${entryType}`, {
        headers: {
